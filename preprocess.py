...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, filenames_file, fields,
                                      corpus_type, opt):
    '''
    ...
    src_iter = onmt.io.ShardedTextCorpusIterator(
                ...
                "src", opt.max_shard_size, corpus_type)
    tgt_iter = onmt.io.ShardedTextCorpusIterator(
                tgt_corpus, opt.tgt_seq_length_trunc,
                "tgt", opt.max_shard_size, corpus_type,
                assoc_iter=src_iter)

    documents_passed_so_far = 0
    index = 0
    while not src_iter.hit_end():
        ...
        dataset = onmt.io.TextDataset(
                ...
                dynamic_dict=opt.dynamic_dict, filenames_path=filenames_file, documents_passed_so_far=documents_passed_so_far)
        documents_passed_so_far += len(dataset)
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if corpus_type == 'train':
        ...
        filenames_file = opt.train_filenames
    else:
        src_corpus = opt.valid_src
        tgt_corpus = opt.valid_tgt
        filenames_file = opt.valid_filenames

    # Currently we only do preprocess sharding for corpus: data_type=='text'.
    if opt.data_type == 'text':
        return build_save_text_dataset_in_shards(
                src_corpus, tgt_corpus, filenames_file, fields,
                corpus_type, opt)
