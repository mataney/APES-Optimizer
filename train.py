...
if opt.exp_host != "":
    ...
    if opt.exp in experiments:
        ...
        try:
            #the ones that don't come up in get_experiment_names because they don't have any data points
...
def report_func(epoch, batch, num_batches,
                progress_step,
                start_time, lr, report_stats, prefix="progress"):
    """
    ...
    if batch % opt.report_every == -1 % opt.report_every:
        ...
        if opt.exp_host:
            report_stats.log(prefix, experiment, lr)
        if opt.tensorboard:
            # Log the progress using the number of batches on the x-axis.
            report_stats.log_tensorboard(
                prefix, writer, lr, progress_step)
...
def train_model(model, fields, optim, data_type, model_opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        train_iter = make_dataset_iter(lazily_load_dataset("train"),
                                       fields, opt)
        train_stats, train_ee_stats = trainer.train(train_iter, epoch, report_func)
        ...
        valid_iter = make_dataset_iter(lazily_load_dataset("valid"),
                                       fields, opt,
                                       is_train=False)
        valid_stats, valid_ee_stats = trainer.validate(valid_iter)
        ...
        if opt.exp_host:
            ...
            train_ee_stats.log("train_encoder_entities", experiment, optim.lr)
            valid_stats.log("valid", experiment, optim.lr)
            valid_ee_stats.log("valid_encoder_entities", experiment, optim.lr)
        if opt.tensorboard:
...
def main():
    ...
    if opt.train_from:
        ...
        checkpoint['optim'].lr = opt.learning_rate
        checkpoint['optim'].adagrad_accum = opt.adagrad_accumulator_init
