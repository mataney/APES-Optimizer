...
class CopyGenerator(nn.Module):
    ...
    * :math:`p_{copy}` the probility of copying a word instead.
    ...
    """
    def __init__(self, input_size, tgt_dict, sigmoid=False):
...
class CopyGeneratorLossCompute(onmt.Loss.LossComputeBase):
    ...
    def _make_shard_state(self, batch, output, range_, attns, ent_data):
        ...
        return {
            ...
            "enc_ent_context": ent_data[0][0],
            "enc_ent_attn": ent_data[0][1],
            "src_ent_pos": ent_data[0][2]
        }

    def _compute_loss(self, batch, iteration, output, target, copy_attn, align, 
                            enc_ent_context, enc_ent_attn, src_ent_pos):
        """
        ...
        if iteration == 0: # not sure I need this
            enc_ent_loss = F.binary_cross_entropy(enc_ent_attn, src_ent_pos, size_average=False)
            enc_entities_stats = self._entities_stats(enc_ent_loss.data.clone(), enc_ent_attn.data, src_ent_pos.data)
        ...
        return loss, stats, enc_entities_stats
