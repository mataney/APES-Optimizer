...
questions_mapping = read_pickle('/path/to/questions/mapping.pkl')
...
class TextDataset(ONMTDatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
                 ...
                 dynamic_dict=True, use_filter_pred=True, filenames_path=None, documents_passed_so_far=0):
        self.data_type = 'text'
        ...
        if filenames_path is not None:
            ...
            examples_iter = (self._join_dicts(ex, {'article_hash': filenames[documents_passed_so_far+i]})
                        for i, ex in enumerate(examples_iter))
    ...
    def get_fields(n_src_features, n_tgt_features):
        ...
        fields["article_hash"] = torchtext.data.RawField()
        fields["entities_ids"] = torchtext.data.RawField()
        fields["src_ent_pos"] = torchtext.data.Field(
            use_vocab=False, tensor_type=torch.LongTensor,
            postprocessing=make_tgt, sequential=False)
    ...
    def _dynamic_dict(self, examples_iter):
        for example in examples_iter:
            ...
            if 'article_hash' in example:
                # questions_entities = questions_mapping[example['article_hash']]['questions_entities']
                questions_entities = [q['answer'] for q in questions_mapping[example['article_hash']]['questions'].values()]
                entities_and_names = [[e, questions_mapping[example['article_hash']]['mapping'][e]] for e in questions_entities]
                entities_and_names = sorted(entities_and_names, key = lambda item : len(item[1]))
                example["entities_ids"] = {}
                for i in range(len(entities_and_names)):
                    name = entities_and_names[i][1].lower()
                    entity_index = src_vocab.stoi[name]
                    if entity_index != 0:
                        if entity_index not in example["entities_ids"]:
                            example["entities_ids"][entity_index] = 0
                        example["entities_ids"][entity_index] += 1
                    else:
                        parts = name.split()
                        for part in parts:
                            part_entity_index = src_vocab.stoi[part]
                            if part_entity_index != 0:
                                if part_entity_index not in example["entities_ids"]:
                                    example["entities_ids"][part_entity_index] = 0
                                example["entities_ids"][part_entity_index] += 1/len(parts)

                src_ent_pos = torch.zeros(len(src_map))
                for entity in example["entities_ids"].keys():
                    src_ent_pos += (src_map == entity).float()
                example["src_ent_pos"] = src_ent_pos
...
class ShardedTextCorpusIterator(object):
    ...
    def __iter__(self):
        ...
        """
        empty_train_source_docs = [598, 611, 676, 687, 986, 1276, 1339, 1561, 1640, 1825, 1903, 2044, 2198, 2727, 2941, 2956, 3056, 3152, 3978, 4192, 4345, 4358, 4409, 4866, 4930, 5110, 5228, 5402, 6429, 6556, 6722, 6962, 7235, 8753, 8770, 9347, 13161, 13562, 13563, 13564, 13565, 13566, 13567, 13568, 13569, 13570, 13571, 13572, 13573, 13574, 13936, 13937, 13938, 13998, 14332, 15313, 15314, 15419, 15420, 15428, 15501, 15526, 15898, 15903, 16108, 16111, 16113, 16114, 16115, 19413, 20028, 20029, 20104, 21622, 21958, 22200, 23376, 24014, 24314, 24350, 24363, 24395, 26300, 27149, 29611, 30677, 30773, 30838, 31400, 32753, 33415, 34726, 40501, 41440, 43685, 45040, 45369, 49937, 53760, 54642, 55890, 56591, 56676, 58341, 58447, 65483, 67114, 67189, 69903, 72920, 72942, 81686, 83574, 83924]
        iteration_index = -1
        if self.assoc_iter is not None:
            ...
            while self.line_index < self.assoc_iter.line_index:
                ...
                self.line_index += 1
                iteration_index += 1
                if self.corpus_type == "train" and self.line_index in empty_train_source_docs: iteration_index-=1; continue
